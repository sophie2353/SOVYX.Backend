const metaService = {
  ...
  async buscarBorrador(adAccountId, token, nombreBorrador) {
    const url = `https://graph.facebook.com/v19.0/${adAccountId}/campaigns?fields=id,name,status&access_token=${token}`;
  ...
  async inyectarSegmentacionYActivar(token, campaignId, dataSegmentacion) {
    ...
    const adSetUrl = `https://graph.facebook.com/v19.0/${campaignId}/adsets?access_token=${token}`;
    const resSet = await fetch(adSetUrl);
    const dataSet = await resSet.json();
    const adSetId = dataSet.data?.[0]?.id;

    if (adSetId && dataSegmentacion) {
      await fetch(`https://graph.facebook.com/v19.0/${adSetId}?access_token=${token}`, {
        method: 'POST',
  ...
  async pausarCampana(token, campaignId) {
    await fetch(`https://graph.facebook.com/v19.0/${campaignId}?access_token=${token}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  async obtenerMetricas(token, campaignId) {
    const url = `https://graph.facebook.com/v19.0/${campaignId}/insights?fields=impressions,clicks,cpc,ctr,spend&access_token=${token}`;
    const res = await fetch(url);
    const data = await res.json();
    return data.data?.[0] || {};
  }
